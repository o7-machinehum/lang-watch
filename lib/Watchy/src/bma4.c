 ...
 *
 */
static void unpack_accel_data(struct bma4_accel *accel_data, uint16_t data_start_index,
                              const struct bma4_dev *dev);
/*!
 ...
 *  @retval Any non zero value -> Fail
 */
static uint16_t stream_transfer_write(const uint8_t *stream_data, uint16_t index,
                                      struct bma4_dev *dev);
 ...
 *
 */
static void scale_offset(uint8_t res, uint8_t range, const struct offset_delta *delta,
...
static void comp_for_grvty(uint16_t lsb_per_g, const int32_t g_val[3],
                           const struct bma4_accel *data, struct offset_delta *comp_data);
/*!
 ...
 * @retval 0 if success, else fail
 */
static uint16_t bma4_extract_aux_data(uint8_t aux_reg_addr, uint8_t *aux_data, uint16_t len,
                                      struct bma4_dev *dev);
...
static uint16_t write_regs(uint8_t addr, uint8_t *data, uint8_t len, const struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (dev->interface == BMA4_SPI_INTERFACE) addr = addr & BMA4_SPI_WR_MASK;
...
uint16_t bma4_get_temperature(int32_t *temp, uint8_t temp_unit, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) temp_raw_scaled = (int32_t)data[BMA4_TEMP_BYTE] * BMA4_SCALE_TEMP;
        ...
        switch (temp_unit) {
            ...
            case BMA4_FAHREN:
                ...
                (*temp) =
                    (((*temp) / BMA4_SCALE_TEMP) * BMA4_FAHREN_SCALED) + (32 * BMA4_SCALE_TEMP);
                break;
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) {
            ...
            if (rslt == BMA4_OK) {
                ...
                rslt |=
                    bma4_write_regs((BMA4_ACCEL_CONFIG_ADDR + 1), &accel_config_data[1], 1, dev);
            }
        }
    }
...
uint16_t bma4_get_advance_power_save(uint8_t *adv_pwr_save, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *adv_pwr_save = BMA4_GET_BITS_POS_0(data, BMA4_ADVANCE_POWER_SAVE);
    }
...
uint16_t bma4_get_fifo_self_wakeup(uint8_t *fifo_self_wake_up, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *fifo_self_wake_up = BMA4_GET_BITSLICE(data, BMA4_FIFO_SELF_WAKE_UP);
    }
...
uint16_t bma4_get_accel_enable(uint8_t *accel_en, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *accel_en = BMA4_GET_BITSLICE(data, BMA4_ACCEL_ENABLE);
    }
...
uint16_t bma4_get_mag_enable(uint8_t *mag_en, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *mag_en = BMA4_GET_BITS_POS_0(data, BMA4_MAG_ENABLE);
    }
...
uint16_t bma4_get_spi_interface(uint8_t *spi, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *spi = BMA4_GET_BITS_POS_0(data, BMA4_CONFIG_SPI3);
    }
...
uint16_t bma4_set_mag_manual_enable(uint8_t mag_manual, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) {
            ...
            if (rslt == BMA4_OK) dev->aux_config.manual_enable = (uint8_t)mag_manual;
        } else {
...
uint16_t bma4_get_mag_manual_enable(uint8_t *mag_manual, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *mag_manual = BMA4_GET_BITSLICE(data, BMA4_MAG_MANUAL_ENABLE);
    }
...
uint16_t bma4_get_mag_read_addr(uint8_t *mag_read_addr, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *mag_read_addr = BMA4_GET_BITS_POS_0(data, BMA4_READ_ADDR);
    }
...
uint16_t bma4_get_mag_write_addr(uint8_t *mag_write_addr, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *mag_write_addr = BMA4_GET_BITS_POS_0(data, BMA4_WRITE_ADDR);
    }
...
uint16_t bma4_get_mag_write_data(uint8_t *mag_write_data, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *mag_write_data = BMA4_GET_BITS_POS_0(data, BMA4_WRITE_DATA);
    }
...
uint16_t bma4_get_mag_burst(uint8_t *mag_burst, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *mag_burst = BMA4_GET_BITS_POS_0(data, BMA4_MAG_BURST);
    }
    ...
    if (dev == NULL || dev->fifo == NULL || dev->fifo->data == NULL) {
        ...
        if (dev->fifo->fifo_header_enable == 0) {
            ...
            for (; data_index < data_read_length;) {
                unpack_acc_frm(accel_data, &data_index, &accel_index, dev->fifo->fifo_data_enable,
                               dev);
...
uint16_t bma4_get_fifo_wm(uint16_t *fifo_wm, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (BMA4_OK == rslt) *fifo_wm = (data[1] << 8) | (data[0]);
    }
...
uint16_t bma4_get_accel_fifo_filter_data(uint8_t *accel_fifo_filter, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *accel_fifo_filter = BMA4_GET_BITSLICE(data, BMA4_FIFO_FILTER_ACCEL);
    }
...
uint16_t bma4_get_fifo_down_accel(uint8_t *fifo_down, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *fifo_down = BMA4_GET_BITSLICE(data, BMA4_FIFO_DOWN_ACCEL);
    }
...
uint16_t bma4_get_if_mode(uint8_t *if_mode, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *if_mode = BMA4_GET_BITSLICE(data, BMA4_IF_CONFIG_IF_MODE);
    }
...
uint16_t bma4_get_accel_data_rdy(uint8_t *data_rdy, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *data_rdy = BMA4_GET_BITSLICE(data, BMA4_STAT_DATA_RDY_ACCEL);
    }
...
uint16_t bma4_get_mag_data_rdy(uint8_t *data_rdy, struct bma4_dev *dev) {
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) *data_rdy = BMA4_GET_BITSLICE(data, BMA4_STAT_DATA_RDY_MAG);
    }
    ...
    if (dev == NULL) {
        ...
        if (rslt == BMA4_OK) {
            ...
            if (rslt == BMA4_OK) {
                ...
                if (rslt == BMA4_OK) *result = BMA4_SELFTEST_PASS;
...
uint16_t bma4_aux_interface_init(struct bma4_dev *dev) {
    ...
    if (rslt == BMA4_OK) {
        ...
        if (rslt != BMA4_OK) rslt = BMA4_E_AUX_CONFIG_FAIL;
    }
...
static void comp_for_grvty(uint16_t lsb_per_g, const int32_t g_val[3],
    ...
    comp_data->x.val =
        (int16_t)(data->x - (int16_t)((accel_value_lsb[BMA4_X_AXIS] / (int64_t)BMA4XY_MULTIPLIER)));
    comp_data->y.val =
        (int16_t)(data->y - (int16_t)((accel_value_lsb[BMA4_Y_AXIS] / (int64_t)BMA4XY_MULTIPLIER)));
    comp_data->z.val =
        (int16_t)(data->z - (int16_t)((accel_value_lsb[BMA4_Z_AXIS] / (int64_t)BMA4XY_MULTIPLIER)));
...
static void get_accel_len_to_parse(uint16_t *start_idx, uint16_t *len, const uint16_t *acc_count,
    ...
    if (dev->fifo->accel_byte_start_idx == 0) dummy_byte_spi = dev->dummy_byte;
...
static void extract_accel_header_mode(struct bma4_accel *accel_data, uint16_t *accel_length,
    ...
    /*Check if this is the first iteration of data unpacking
     if yes, then consider dummy byte on SPI*/
    if (dev->fifo->accel_byte_start_idx == 0) dev->fifo->accel_byte_start_idx = dev->dummy_byte;
...
static void get_mag_len_to_parse(uint16_t *start_idx, uint16_t *len, const uint16_t *mag_count,
    ...
    /*Check if this is the first iteration of data unpacking
     if yes, then consider dummy byte on SPI*/
    if (dev->fifo->mag_byte_start_idx == 0) dummy_byte_spi = dev->dummy_byte;
...
static uint16_t extract_mag_header_mode(struct bma4_mag *data, uint16_t *len,
    ...
    /*Check if this is the first iteration of data unpacking
     if yes, then consider dummy byte on SPI*/
    if (dev->fifo->mag_byte_start_idx == 0) dev->fifo->mag_byte_start_idx = dev->dummy_byte;
...
static int32_t power(int16_t base, uint8_t resolution) {
    ...
    for (; i <= resolution; i++) value = (int32_t)(value * base);
